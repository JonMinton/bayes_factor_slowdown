...
bf_table_part <- 
  bf_mutexc %>% 
    ...
    select(-period, -ll) %>% 
    gather(-sex, -year, key = "key", value = "value") %>% 
    unite(sex, key, col = "sex_key") %>% 
    spread(sex_key, value) %>% 
    select(year, female_mu, female_perc, female_bayes_factor, male_mu, male_perc, male_bayes_factor)
...
bf_table_part %>% 
  ...
  select(year = year, 
         female_bf = female_mu, female_ons = female_ons_ch_e0,  female_bayes_factor,
         male_bf = male_mu, male_ons = male_ons_ch_e0, male_bayes_factor
  ) %>% 
  set_names(nm = c("Projection Year", rep(c("Bayes Factor Estimate", "ONS Estimate", "Bayes Factor"), 2)))
